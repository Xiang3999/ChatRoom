...
def in_room(user_id, room_id):
    c = get_cursor()
    r = c.execute('SELECT 1 FROM room_list WHERE user_id=? AND room_id=? ',
                  [user_id, room_id]).fetchall()
    return len(r) > 0

def add_to_room(user_id, room_id):
    c = get_cursor()
    r = c.execute('INSERT INTO room_list (user_id,room_id) VALUES (?,?) ',
                  [user_id, room_id])

def get_room_members_id(room_id):
    return list(map(lambda x: x[0], get_cursor().execute('SELECT user_id FROM room_list WHERE room_id=?',
                                                         [room_id]).fetchall()))
